...
<div class="bg-white p-6 rounded-lg shadow max-w-2xl">
    ...
    <form method="POST" action="{{ route('supplier.produk.update', $produk->id) }}" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="space-y-4">
            <div>
            ...
            <div class="grid grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="number" name="price" value="{{ old('price', $produk->price) }}" step="0.01" min="0" class="w-full border rounded px-3 py-2 focus:ring-2 focus:ring-orange-500 focus:border-transparent" placeholder="0.00" required>
                </div>
                <div>
                    <label class="block mb-1 font-medium text-gray-700">Stok <span class="text-red-500">*</span></label>
                    <input type="number" name="stock" value="{{ old('stock', $produk->stock) }}" min="0" class="w-full border rounded px-3 py-2 focus:ring-2 focus:ring-orange-500 focus:border-transparent" placeholder="0" required>
                </div>
                <div>
                    <label class="block mb-1 font-medium text-gray-700">Min. Pembelian <span class="text-red-500">*</span></label>
                    <input type="number" name="min_order" value="{{ old('min_order', $produk->min_order ?? 1) }}" min="1" class="w-full border rounded px-3 py-2 focus:ring-2 focus:ring-orange-500 focus:border-transparent" placeholder="1" required>
            ...
            <div>
                ...
                @if($produk->primaryImage)
                    <div class="mb-3">
                        <img src="{{ asset('storage/' . $produk->primaryImage->path) }}" alt="{{ $produk->name }}" class="w-32 h-32 object-cover rounded border">
